...
| [018](decisions/adr-018-ambient-failure-detector.md) | Ambient `failureDetector` ((Any?)→Throwable?): treat a *returned* failure value like a thrown one (span-level ERROR + birthplace), process-global on `Kotrace` + per-call override; root detector supplies the default trace verdict by precedence — amends ADR-016 | Accepted | 2026-09-19 |
